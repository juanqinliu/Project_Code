#include "ffmpeg/bs_push_streamer.h"
...
bool BsPushStreamer::connect(std::string name, int width, int height, int fps, std::string encoder, int bitrate)
{
    ...
    // Compress the video bit size 300kB
    ...
    // Not support AV_PIX_FMT_BGR24 directly for encoding
    ...
    // The time_base parameter of the stream is very important, it represents how many time bases are in one second, and is automatically completed when calling avformat_write_header below
    ...
    av_dict_set(&fmt_options, "rw_timeout", "30000000", 0); // Set the timeout for the rtmp/http-flv connection (unit us)
    av_dict_set(&fmt_options, "stimeout", "30000000", 0);   // Set the timeout for the rtsp connection (unit us)
    ...
    // Call this function will automatically set a value for all stream's time_base, usually 1/90000 or 1/1000, this represents the length of the time base in one second
...
void BsPushStreamer::encodeVideoAndWriteStreamThread(void* arg)
{
    ...
    // The video frame (bgr format) that is not encoded
    // VideoFrame *videoFrame = NULL;
    // The current length of the video frame queue that is not encoded
    ...
    // The video frame that is encoded
    ...
    while (mBsPushStreamer->push_running)
    {
        ...
        if (mBsPushStreamer->nd_push_frame)
        {
            ...
            // Convert frame_bgr to frame_yuv420p
            ...
            if (ret >= 0)
            {
                ...
                if (ret >= 0)
                {
                    ...
                    // If the actual stream is a flv file, the fix_packet_pts inside will not be executed
    ...
    // av_write_trailer(mFmtCtx);  // Write the file trailer
