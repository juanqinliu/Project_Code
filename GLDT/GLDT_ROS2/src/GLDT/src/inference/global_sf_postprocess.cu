...
__global__ void transformBoxesKernel(
    ...
    if (!isfinite(x) || !isfinite(y) || !isfinite(w) || !isfinite(h)) {
        // If coordinate is invalid, set to zero region box
        box[0] = 0.0f;
        box[1] = 0.0f;
        box[2] = 0.0f;
        box[3] = 0.0f;
    ...
    // Store back result (变为x1,y1,x2,y2格式)
    box[0] = x1;
    box[1] = y1;
    box[2] = x2;
    box[3] = y2;
...
__global__ void nmsKernel(
    ...
    int* indices,         // [num_boxes] input is index, output is also index
...
std::vector<Detection> parseYOLOOutputGPU(
    ...
    try {
        ...
        CUDA_CHECK(cudaMalloc(&d_valid_boxes, max_detections * 5 * sizeof(float)));  // Each box has 5 values
