...
__global__ void transformLocalBoxesKernel(
    float* boxes,        // [num_boxes, 4] format (x,y,w,h)
    int num_boxes,
    int img_width,
    int img_height,
    int input_width,
    ...
    // Store result (change to x1,y1,x2,y2 format)
    box[0] = x1;
    box[1] = y1;
    box[2] = x2;
    box[3] = y2;
...
__global__ void extractLocalDetectionsKernel(
    ...
    // Atomic increase counter and get index, ensure not exceed maximum detection box quantity
...
std::vector<Detection> parseLocalYOLOOutputGPU(
    ...
    try {
        ...
        try {
            ...
            // Allocate memory for valid detections (retain at most max_detections boxes)
            ...
            // Convert box coordinates (using local detection conversion function)
            try {
            ...
            for (int i = 0; i < valid_count; i++) {
                if (indices[i] != -1) {
                    ...
                    if (scores[box_idx] < 0) continue;  // Skip boxes marked as invalid
...
// Parse directly from device output, avoid copying the entire GPU->CPU block
std::vector<Detection> parseLocalYOLOOutputGPUFromDevice(
    ...
    // Record device memory to be released
    std::vector<void*> allocated_memory;
    ...
    try {
        ...
        // Allocate device memory for valid boxes/counts
        ...
        // Extract valid detections (directly use device output)
        int block_size = 128;
        int grid_size = (num_boxes + block_size - 1) / block_size;
        ...
        // Convert coordinates to original image scale (using existing GPU function, internally sync)
        ...
        // Reuse existing conversion (using default stream sync), no impact on accuracy; for further optimization, can be changed to stream version
        ...
        // GPU NMS (existing implementation uses default stream and device sync)
