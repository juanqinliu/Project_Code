...
bool GStreamerVideoReader::open(const std::string& filename) {
    ...
    if (is_v4l2) {
        ...
        // 2) filesrc+decodebin（Explicit filesrc）
        candidates.push_back(
        ...
        // 5) H.264 explicit decode chain（Prefer NVDEC/VAAPI, then fallback to CPU）
        candidates.push_back(
            std::string("filesrc location=\"") + filename +
        ...
        // 6) H.265/HEVC explicit decode chain（Prefer NVDEC/VAAPI, then fallback to CPU）
        candidates.push_back(
            std::string("filesrc location=\"") + filename +
