    ...
    int dst_height, int edge)
{
  ...
  int dx = position % dst_width; // Calculate x coordinate of target image for current thread
  int dy = position / dst_width; // Calculate y coordinate of target image for current thread

  // normalization (normalize 3 channels of pixel at (x,y) in original image)
    ...
    AffineMatrix d2s, int edge)
{
  ...
  // Read transformation matrix from d2s
  ...
  {
    // Fill pixels outside boundary with const_value_st
    c0 = const_value_st;
    c1 = const_value_st;
    c2 = const_value_st;
  }
...
// GPU performs normalization, BGR2RGB, NHWC to NCHW
    ...
    uint8_t *src, float *dst, int dst_width, int dst_height)
{
  ...
  // One thread processes one pixel, total dst_height * dst_width threads needed
...
// Use CUDA to preprocess all steps
...
// Perform letterbox processing on input image: maintain original aspect ratio, place image on square canvas, fill extra parts with black
// Use cv::warpAffine to perform affine transformation, placing image on canvas
// The inline keyword here means this function will be directly replaced at call site during compilation, avoiding function call overhead for efficiency
inline cv::Mat letterbox(cv::Mat &src)
{
  ...
  cv::Point2f srcTri[3]; // Calculate three points of original image: top-left, top-right, bottom-left
  srcTri[0] = cv::Point2f(0.f, 0.f);
  srcTri[1] = cv::Point2f(src.cols - 1.f, 0.f);
  srcTri[2] = cv::Point2f(0.f, src.rows - 1.f);
  cv::Point2f dstTri[3]; // Calculate three points of target image: top-left, top-right, bottom-left
...
// CPU performs letterbox, GPU performs normalization, BGR2RGB, NHWC to NCHW
...
// CPU performs letterbox, normalization, BGR2RGB, NHWC to NCHW
void process_input_cpu(cv::Mat &src, float *input_device_buffer)
{
  ...
  cv::split(warp_dst, warp_dst_nchw_channels); // Decompose input image into three single-channel images: rrrrr, ggggg, bbbbb

  // Reshape each single-channel image to 1x1xHxW four-dimensional matrix
  for (auto &img : warp_dst_nchw_channels)
  {
    // Reshape parameters are cn: number of channels, rows: number of rows
    // Similar to [[r,r,r,r,r]] or [[g,g,g,g,g]] or [[b,b,b,b,b]], each with width * height elements
    img = img.reshape(1, 1);
  }
  // Concatenate three single-channel images into one three-channel image: rrrrr, ggggg, bbbbb concatenated to rrrgggbbb
  cv::Mat warp_dst_nchw;
  cv::hconcat(warp_dst_nchw_channels, warp_dst_nchw);
  // Copy processed image data to GPU
