...
# Global Detection PT Model to TensorRT Engine Export Script (One-Step)
...
show_help() {
    echo "Global Detection PT Model to TensorRT Engine Export Script (One-Step)"
    echo ""
    echo "This script combines PT->ONNX->Engine conversion into a single command."
    echo ""
    echo "Usage:"
    ...
    echo ""
    echo "  # Static ONNX with custom output path"
    echo "  $0 -i weights/model.pt --static -o model_engine/model.engine"
    echo ""
...
check_environment() {
    ...
    local python_deps=("torch" "onnx" "ultralytics" "tensorrt" "pycuda" "numpy" "opencv-python")
    ...
    if [ ${#missing_deps[@]} -gt 0 ]; then
        ...
        echo "  pip3 install torch onnx ultralytics tensorrt pycuda numpy opencv-python"
        return 1
    fi
...
convert_onnx_to_engine() {
    ...
    local cmd=("$EXPORT_ENGINE_SCRIPT" "-i" "$onnx_file")
    
    if [ -n "$engine_file" ]; then
        cmd+=("--output" "$engine_file")
    fi
