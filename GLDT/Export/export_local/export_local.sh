...
MAX_BATCH=32
WORKSPACE=2048
CALIB_DIR=""  # Will be set after SCRIPT_DIR is determined
CALIB_LIST=""  # Will be set after SCRIPT_DIR is determined
PRECISION="int8"
AUTO_CALIB=false
CALIB_NUM=1000
...
show_help() {
    ...
    echo "  --calib-dir <dir>         Calibration data directory (default: ./export_engine/calib_data)"
    echo "  --calib-list <file>       Calibration list file (default: ./export_engine/calib_list.txt)"
...
# Set default calibration paths if not specified by user
if [[ -z "$CALIB_DIR" ]]; then
    CALIB_DIR="$SCRIPT_DIR/export_engine/calib_data"
fi
if [[ -z "$CALIB_LIST" ]]; then
    CALIB_LIST="$SCRIPT_DIR/export_engine/calib_list.txt"
fi

# Step 1: PT -> ONNX Conversion
print_info "=== Step 1: PT -> ONNX Conversion ==="

PT_TO_ONNX_SCRIPT="$SCRIPT_DIR/export_onnx/export_onnx.sh"
...
ONNX_TO_ENGINE_SCRIPT="$SCRIPT_DIR/export_engine/export_engine.sh"
...
if [[ "$PRECISION" == "int8" ]]; then
    ...
    # If user explicitly set --auto-calib-list or --calib-num, pass them through
    if [[ "$AUTO_CALIB" == true ]]; then
        ENGINE_ARGS+=(--auto-calib-list)
    fi
    
    # Always pass --calib-num if explicitly set, or if --auto-calib-list is used
    if [[ "$CALIB_NUM_SET" == true || "$AUTO_CALIB" == true ]]; then
